...
export default function PlacementHeader({ gameState, onAbort }: PlacementHeaderProps) {
  ...
  return (
    <header className="relative flex items-center justify-between shrink-0 py-2 border-b border-white/5 mb-8">
           ...
           <div className="flex items-baseline gap-3">
              <h1 className="text-xl font-black text-white uppercase tracking-tighter">
                 {(gameState.opponent || gameState.gameMode === 'PvE') 
                   ...
                   : (gameState.roomId ? '' : <>{gameState.playerName || 'Commander'} <span className="text-slate-600 mx-1">VS</span> Opponent</>)
                 }
              </h1>
      ...
      {gameState.roomId && (
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center gap-3 bg-slate-900/50 backdrop-blur border border-white/10 px-6 py-2 rounded-full shadow-lg z-0">
        ...
        <div className="flex flex-col items-end">
          ...
          <span className="text-xl font-black uppercase italic tracking-tight leading-none text-primary glow-primary">
            {gameState.gameMode === 'PvE' 
              ? t('status_training') 
              : ((gameState.roomId && (gameState.isFleetReady || gameState.opponent?.fleetReady)) ? t('status_waiting') : t('status_waiting'))
            }
          </span>
        </div>
