...
const LobbyChat = ({ onlineUsers, t }: LobbyChatProps) => {
  ...
  if (!isOpen) {
    return (
      <button 
        onClick={toggleChat}
        className="fixed bottom-6 right-6 z-50 w-14 h-14 bg-primary hover:bg-blue-600 rounded-full flex items-center justify-center shadow-[0_0_20px_rgba(25,93,230,0.5)] hover:shadow-[0_0_30px_rgba(25,93,230,0.8)] transition-all hover:scale-110 active:scale-95 group"
      >
        <MessageSquare className="w-7 h-7 text-white" />
        <span className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 rounded-full text-[10px] font-black text-white flex items-center justify-center border-2 border-[#0a0e1a]">
          {onlineUsers}
        </span>
      </button>
    );
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-2">
        ...
        {/* Toggle Button (Visible when open too, to minimize) */}
        <button 
            onClick={toggleChat}
            className="w-14 h-14 bg-slate-800 hover:bg-slate-700 rounded-full flex items-center justify-center border border-white/10 shadow-xl transition-all"
        >
