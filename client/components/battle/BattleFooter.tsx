...
export default function BattleFooter({ accuracy, sunkEnemyShips }: BattleFooterProps) {
  const { t } = useLanguage();
  return (
    <footer className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-6 pt-4 border-t border-white/5 pb-6 lg:pb-0">
       <div className="flex items-center gap-4 px-4 py-2 bg-slate-900/20 rounded-xl border border-white/5">
          <span className="text-[10px] md:text-xs font-black text-slate-500 uppercase tracking-widest">{t('accuracy')}</span>
       ...
       <div className="flex items-center gap-4 px-4 py-2 bg-slate-900/20 rounded-xl border border-white/5">
          <span className="text-[10px] md:text-xs font-black text-slate-500 uppercase tracking-widest">{t('enemy_ships')}</span>
       ...
       <div className="hidden md:flex items-center gap-4 px-4 py-2 bg-primary/5 rounded-xl border border-primary/10 relative overflow-hidden group">
          <span className="text-[10px] md:text-xs font-black text-slate-500 uppercase tracking-widest">{t('momentum')}</span>
          <p className="text-[10px] md:text-sm font-black text-primary uppercase italic ml-2 group-hover:translate-x-1 transition-transform truncate">{t('advantage_you')}</p>
